...
sort(tapply(d$POUNDS_LANDED, d$ITIS_COMMON_NAME, sum, na.rm = T))
...
totland_pr <- tapply(d$POUNDS_LANDED, list(d$YEAR_LANDED, d$sppgrp), sum, na.rm = T) / 10^3
dim(totland_pr)
totland_pr
...
plotIndicatorTimeSeries(s, coltoplot = 1:6, plotrownum = 2, sublabel = T, sameYscale = F, 
                        widadj = 0.8, hgtadj = 0.8, outtype = "png")
