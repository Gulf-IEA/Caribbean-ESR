...
save(s, file = "landingsDisturbance.RData")

plotIndicatorTimeSeries(s, sublabel = T) #, outtype = "png")
...
# format as indicator object and plot -----------------
load("landingsDisturbance.RData")

yrind <- which(s$datelist %in% yrs)

inddata <- data.frame(avdst)
llidata <- data.frame(avdst - sddst)
ulidata <- data.frame(avdst + sddst)
labs <- c("Disturbance indicator", "difference from mean landings", "St. Thomas and St. John")
indnames <- data.frame(matrix(labs, nrow = 3, byrow = T))

s$labels <- data.frame(cbind(s$labels, indnames))
s$indicators[,2] <- NA
s$indicators[yrind, 2] <- inddata
s$ulim[,2] <- NA
s$ulim[yrind, 2] <- ulidata
s$llim[,2] <- NA
s$llim[yrind, 2] <- llidata

plotIndicatorTimeSeries(s, sublabel = T, coltoplot = 1:2, plotrownum = 2, yposadj = 1.2, type = "allLines", outtype = "png", widadj = 0.7)
